...
struct path_data {
    ...
    void update_tab_content() { tab_content = get_file_name(file_path); }
};
...
inline auto get_menu(const std::shared_ptr<path_data> &input_path_data) {
    ...
    return multiselect_menu(
        &input_path_data->menu.entries, input_path_data->menu.selected.get(), menu_option);
}
...
inline auto add_folder_menu(
    std::vector<std::shared_ptr<path_data>> &path_datas, Component &tab_container, int index) {
...
inline auto remove_folder_menu(
    std::vector<std::shared_ptr<path_data>> &path_datas, Component &tab_container, int index) {
    if (path_datas.empty()) {
